---
title: "NVIDIA-AI-Blueprints/ai-virtual-assistant: Customizable, AI-driven virtual assistant designed to streamline customer service operations, handle common inquiries, and improve overall user satisfaction through automated and contextually aware responses."
source: "https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant"
author:
published:
created: 2026-04-05
description: "Customizable, AI-driven virtual assistant designed to streamline customer service operations, handle common inquiries, and improve overall user satisfaction through automated and contextually aware responses. - NVIDIA-AI-Blueprints/ai-virtual-assistant"
tags:
  - "clippings"
---
## NVIDIA AI Blueprint: AI Virtual Assistant for Customer Service
...
This repository is what powers the [build experience](https://build.nvidia.com/nvidia/ai-virtual-assistant-for-customer-service), showcasing an AI virtual assistant with NVIDIA NIM microservices ([https://build.nvidia.com/nim](https://build.nvidia.com/nim))
...
- NVIDIA NIM microservices
	- Response Generation (Inference)
		...
		- [Synthetic Data Generation](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/notebooks/synthetic_data_generation.ipynb) for reference
		- [NIM of Nemotron4-340B](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/nvidia/nemotron-4-340b-instruct)
...
Below are the hardware requirements for each component. The reference code in the solution (glue code) is referred to as as the "pipeline".
...
[![](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/raw/main/docs/imgs/IVA-blueprint-diagram-r5.png)](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/docs/imgs/IVA-blueprint-diagram-r5.png)
...
**Summary** In summary, this NVIDIA NIM Agent Blueprint offers a comprehensive solution for building intelligent, generative AI-powered virtual assistants for customer service, leveraging structured and unstructured data to deliver personalized and efficient support. It includes all necessary tools and guidance to deploy, monitor, and continually improve the solution in real-world environments.
...
There are two supported configurations for starting the Docker containers. The default passwords in [docker-compose.yaml](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/deploy/compose/docker-compose.yaml) and the source code (e.g. default DB passwords) should be changed before deploying the blueprint.
...
```
1. [**Self-hosted NIMs**](#using-self-hosted-nims): The blueprint is run using self-hosted LLM NIM services.
```
...
Download the manuals into data/manuals\_pdf folder **Run this script to download the manuals listed in the specified txt file**
...
**Execute the ingest\_data.ipynb notebook**

Follow the cells in the notebook ingest\_data.ipynb to do the following
...
A sample frontend is available to showcase key (but not all) functionalities of this blueprint. The source code of this service is not inlcuded in the blueprint. It is expected that you will use this for a quick test before proceeding to customize the blueprint and use the APIs as part of your differentiating Generative AI application.

The UI microservice is already included in the Docker Compose setup. Visit `http://<HOST-IP>:3001/` to access the frontend and test the suggested queries. You can also refer to the data to create meaningful queries. You can choose any of the preset customer names and ask queries relevant to order history displayed or relevant to any ingested unstructured data.

Click `End Chat Session` button once you have finished your conversation.

The sample UI showcases limited number of functionalities and interacts with a single API endpoint exposed by the [API Gateway container](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/docs/api_references/api_gateway_server.json).

#### Standalone APIs

We expect you to use these APIs to integrate the blueprint with your Generative AI applications For detailed API references of all the key deployed microservices, please refer to the following:

- [Analytics Microservice](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/.docs/api_references/analytics_server.json)
- [Agent Microservice](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/docs/api_references/agent_server.json)
- [Retriever Microservices](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/docs/api_references/retriever_server.json)

The notebooks folder [api\_usage.ipynb](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/notebooks/api_usage.ipynb) illustrates how to use the Agent and the Analytics APIs along with how to extract the user feedback data.
...
The blueprint is a reference example with sample data illustrating how a customer can create an entire solution with NVIDIA NIMs. However it is completely expected that customers will tear apart the solution to pick out what is meaningful in any capacity. Here are some notes as you customize the blueprint
...
- - When bringing your own customer data such as your order history etc. you would need to modify the source code with the relevant [prompts](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/src/agent/prompt.yaml).
...
It can be envisioned that some feedback is provided by the user conversing with the virtual assistant and some feedback will be provided by the customer services operations' team. The feedback scores follow this convention

- Postive indicated by +1
- Neutral indicated by 0
- Negative indicated by -1

Sample code for retrieving the feedback data is in the notebook included [here](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/notebooks).
...
- You have the NGC CLI available on your client machine. You can download the CLI from [https://ngc.nvidia.com/setup/installers/cli](https://ngc.nvidia.com/setup/installers/cli).
- You have Kubernetes installed and running Ubuntu 22.04. Refer to the [Kubernetes documentation](https://kubernetes.io/docs/setup/) or the [NVIDIA Cloud Native Stack repository](https://github.com/NVIDIA/cloud-native-stack/) for more information.
- You have a default storage class available in the cluster for PVC provisioning. One option is the local path provisioner by Rancher. Refer to the [installation](https://github.com/rancher/local-path-provisioner?tab=readme-ov-file#installation) section of the README in the GitHub repository.
	```
...
Create a generic secret for NGC\_API\_KEY
...
Look for the UI service `aiva-aiva-ui`, the port type is listed as NodePort. The NodePort is the one that follows the container port `3001:<nodePort>/TCP`

In this example case, the node port is listed as `32417`, hence the UI can be accessed at [http://0.0.0.0:32417/](http://0.0.0.0:32417/)
...
This error typically indicates that the container is attempting to access GPUs that are either unavailable or non-existent on the host. To resolve this, verify the GPU count specified in the [nims.yaml](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/deploy/compose/nims.yaml) configuration file:
...
We're posting these examples on GitHub to support the NVIDIA LLM community and facilitate feedback. We invite contributions! Open a GitHub issue or pull request! See contributing [guidelines here.](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/CONTRIBUTING.md)

## License

This NVIDIA NIM-AGENT BLUEPRINT is licensed under the [Apache License, Version 2.0.](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/LICENSE.md) Use of the sample data provided as part of this blueprint is governed by [the NVIDIA asset license.](https://github.com/NVIDIA-AI-Blueprints/ai-virtual-assistant/blob/main/data/LICENSE)