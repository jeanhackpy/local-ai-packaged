-- units2_schema_v2.sql
-- Clean table set for unit extraction (Phase 2, fresh start) — v2
-- Adds unit-level traceability to images: floor_area_sqm, price_thb, bedrooms
-- Based on replica schema v3: 63 cols projects, 58 cols units
-- Run: psql $VPS_PG_URL -f units2_schema_v2.sql
...
-- v2: adds unit_url, bedrooms, floor_area_sqm, price_thb for direct querying
...
CREATE TABLE units2_unit_images (
    ...
    project_url          TEXT,
    project_type         TEXT,
    -- unit-level details (denormalised for direct querying without JOIN)
    unit_url             TEXT,
    bedrooms             NUMERIC,
    floor_area_sqm       NUMERIC,
    price_thb            BIGINT
...
CREATE INDEX units2_unit_images_unit_url_idx ON units2_unit_images(unit_url);
CREATE INDEX units2_unit_images_price_thb_idx ON units2_unit_images(price_thb) WHERE price_thb IS NOT NULL;
