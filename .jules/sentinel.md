...
## 2025-05-15 - Information Disclosure in AI Pipeline Errors
**Vulnerability:** Python-based AI agent pipes (`n8n_pipe.py`, `router_pipe.py`) were returning raw exception strings and stack traces to the front-end user interface. This could leak internal architecture details, environment variables, or external API endpoints.
**Learning:** Default error handling in "plug-and-play" pipeline scripts often prioritizes developer convenience over production security, leading to verbose errors in the UI.
**Prevention:** Implement generic error messages for the UI (e.g., "Communication failed") and offload technical details to server-side logs (`sys.stderr` or formal logging). Also, always initialize response variables early to prevent `UnboundLocalError` from crashing the handler before it can return a clean error response.
