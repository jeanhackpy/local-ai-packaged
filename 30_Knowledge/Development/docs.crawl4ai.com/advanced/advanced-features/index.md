...
- Many commercial proxies provide an HTTP/HTTPS “gateway” server that you specify in
server
.
...
url
=
"https://en.wikipedia.org/wiki/List\_of\_common\_misconceptions"
,
cache\_mode
=
CacheMode
...
=
None
# We'll override below for clarity
)
crawler1
.
crawler\_strategy
...
())
Notes
- Some sites may react differently to certain headers (e.g.,
Accept-Language
).
- If you need advanced user-agent randomization or client hints, see
Identity-Based Crawling (Anti-Bot)
or use
UserAgentGenerator
.
...
await context.storage\_state(path="my\_storage.json")
: Exports cookies, localStorage, etc. to a file.
Provide
storage\_state="my\_storage.json"
on subsequent runs to skip the login step.
See
:
Detailed session management tutorial
or
Explanations → Browser Context & Managed Browser
...
=
True
# Will check and respect robots.txt rules
)
async
with
AsyncWebCrawler
...
- Cache is stored in
~/.crawl4ai/robots/robots\_cache.db
...
"my\_storage.json"
,
# Reuse session from a previous sign-in
verbose
=
True
,
...
print
(
"[OK] Crawled the secure page. Links found:"
,
len
(
result
