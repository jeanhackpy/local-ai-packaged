...
def prepare_openclaw_config():
    ...
    if not os.path.exists(example_path):
        print(f"Error: {example_path} not found. Cannot generate OpenClaw config.")
        return

    # 1. Load environment variables
    env_vars = load_env(root_env_path)

    # 2. Check for missing OPENCLAW_GATEWAY_TOKEN and generate if needed
    if "OPENCLAW_GATEWAY_TOKEN" not in env_vars or not env_vars["OPENCLAW_GATEWAY_TOKEN"]:
        print("OPENCLAW_GATEWAY_TOKEN not found in .env. Generating a secure one...")
        new_token = secrets.token_hex(24)
        env_vars["OPENCLAW_GATEWAY_TOKEN"] = new_token

        # Append to .env file
        with open(root_env_path, 'a') as f:
            f.write(f"\n# Auto-generated by start_services.py\nOPENCLAW_GATEWAY_TOKEN={new_token}\n")
    ...
    # 4. Inject values into the configuration
    # Note: We use .get() to avoid KeyError if the example file doesn't have the expected structure
    try:
        # Inject Gateway Token
        if "gateway" in config and "auth" in config["gateway"]:
            config["gateway"]["auth"]["token"] = env_vars.get("OPENCLAW_GATEWAY_TOKEN", "YOUR_GATEWAY_TOKEN")

        # Inject Telegram Bot Token
        if "channels" in config and "telegram" in config["channels"]:
            config["channels"]["telegram"]["botToken"] = env_vars.get("TELEGRAM_BOT_TOKEN", "YOUR_BOT_TOKEN")

        # Inject Workspace Path
        if "agents" in config and "defaults" in config["agents"]:
            config["agents"]["defaults"]["workspace"] = env_vars.get("OPENCLAW_WORKSPACE", "YOUR_WORKSPACE_PATH")
