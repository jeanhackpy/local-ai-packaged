        ...
        with open(example_env_path, 'r') as f:
            for line in f:
                ...
                if line and not line.startswith('#') and '=' in line:
                    ...
                    env_vars[key.strip()] = value.split('#')[0].strip()
    else:
...
def prepare_openclaw_config():
    ...
    if not os.path.exists(example_config_path):
        print(f"Warning: {example_config_path} not found. Skipping OpenClaw config preparation.")
        return
    ...
    if not gateway_token:
        ...
        # Persist the generated token back to .env for stability across restarts
        try:
            with open(root_env_path, 'a') as f:
                f.write(f"\n# Auto-generated by start_services.py\nOPENCLAW_GATEWAY_TOKEN={gateway_token}\n")
            print(f"Successfully persisted new OPENCLAW_GATEWAY_TOKEN to {root_env_path}")
        except Exception as e:
            print(f"Warning: Could not persist gateway token to .env: {e}")
    ...
    try:
        ...
        # Inject tokens into configuration
        ...
        # Ensure gatewayToken is set for models that might need it
        if "agents" in config and "defaults" in config["agents"] and "models" in config["agents"]["defaults"]:
            for model_config in config["agents"]["defaults"]["models"].values():
                if isinstance(model_config, dict):
                    model_config["gatewayToken"] = gateway_token
