...
def prepare_supabase_env():
    ...
    env_vars.update(load_env(example_env_path))

    # 2. Override with values from root .env
    env_vars.update(load_env(root_env_path))
...
def prepare_openclaw_env():
    """Ensure OPENCLAW_GATEWAY_TOKEN is present in .env, generating one if missing."""
    root_env_path = ".env"
    env_vars = load_env(root_env_path)

    if not env_vars.get("OPENCLAW_GATEWAY_TOKEN"):
        print("Generating a new OPENCLAW_GATEWAY_TOKEN...")
        new_token = secrets.token_hex(24)
        try:
            with open(root_env_path, 'a') as f:
                if os.path.exists(root_env_path) and os.path.getsize(root_env_path) > 0:
                    with open(root_env_path, 'rb+') as f_check:
                        f_check.seek(-1, os.SEEK_END)
                        if f_check.read(1) != b'\n':
                            f.write('\n')
                f.write(f"OPENCLAW_GATEWAY_TOKEN={new_token}\n")
            print(f"Added OPENCLAW_GATEWAY_TOKEN to {root_env_path}")
        except Exception as e:
            print(f"Error adding token to {root_env_path}: {e}")
...
def prepare_openclaw_config():
    """Merge .env values into openclaw/openclaw.json.example and write to openclaw/openclaw.json."""
    import json
    ...
    print(f"Generating {target_config_path} from {example_config_path}...")
    ...
    try:
        ...
        print(f"Error loading {example_config_path}: {e}")
        return
    ...
    if "OPENCLAW_GATEWAY_TOKEN" in env_vars and env_vars["OPENCLAW_GATEWAY_TOKEN"]:
        config["gateway"]["auth"]["token"] = env_vars["OPENCLAW_GATEWAY_TOKEN"]
    ...
    try:
        ...
        print(f"Successfully created {target_config_path}")
    except Exception as e:
        print(f"Error writing {target_config_path}: {e}")
