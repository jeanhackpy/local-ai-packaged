...
def prepare_openclaw_config():
    """Merge .env in root with openclaw/openclaw.json.example and write to openclaw/openclaw.json."""
    root_env_path = ".env"
    example_json_path = os.path.join("openclaw", "openclaw.json.example")
    target_json_path = os.path.join("openclaw", "openclaw.json")

    if not os.path.exists(example_json_path):
        print(f"Warning: {example_json_path} not found.")
        return
    ...
    if not env_vars.get("OPENCLAW_GATEWAY_TOKEN"):
        ...
        with open(root_env_path, mode) as f:
            if mode == 'a':
                f.write("\n")
            f.write("# Generated by start_services.py\n")
            f.write(f"OPENCLAW_GATEWAY_TOKEN={gateway_token}\n")
        print(f"Saved new OPENCLAW_GATEWAY_TOKEN to {root_env_path}")
    ...
    print(f"Successfully created {target_json_path}")
