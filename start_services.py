...
def prepare_openclaw_config():
    """Prepare OpenClaw configuration from example template."""
    ...
    if not os.path.exists(config_example_path):
        print(f"Warning: {config_example_path} not found.")
        return
    ...
    if not env_vars.get("OPENCLAW_GATEWAY_TOKEN"):
        print("Generating OPENCLAW_GATEWAY_TOKEN...")
        new_vars["OPENCLAW_GATEWAY_TOKEN"] = secrets.token_hex(24)
        env_vars["OPENCLAW_GATEWAY_TOKEN"] = new_vars["OPENCLAW_GATEWAY_TOKEN"]

    if not env_vars.get("MINIO_ROOT_PASSWORD"):
        print("Generating MINIO_ROOT_PASSWORD...")
        new_vars["MINIO_ROOT_PASSWORD"] = secrets.token_hex(16)
        env_vars["MINIO_ROOT_PASSWORD"] = new_vars["MINIO_ROOT_PASSWORD"]

    if new_vars:
        with open(root_env_path, 'a') as f:
            f.write("\n# Generated by start_services.py\n")
            for key, value in new_vars.items():
                f.write(f"{key}={value}\n")
        print(f"Appended generated secrets to {root_env_path}.")

    # Read template and replace placeholders
    with open(config_example_path, 'r') as f:
        config_content = f.read()

    config_content = config_content.replace("YOUR_TELEGRAM_BOT_TOKEN", env_vars.get("TELEGRAM_BOT_TOKEN", ""))
    config_content = config_content.replace("YOUR_OPENCLAW_GATEWAY_TOKEN", env_vars.get("OPENCLAW_GATEWAY_TOKEN", ""))
