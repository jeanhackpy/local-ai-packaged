...
def prepare_openclaw_config():
    """Generate openclaw/openclaw.json from openclaw/openclaw.json.example with secrets."""
    ...
    print(f"Preparing {target_path} from {example_path}...")
    ...
    if os.path.exists(root_env_path):
        with open(root_env_path, "r") as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    # Robustly handle whitespace and inline comments
                    key_val = line.split("#", 1)[0].strip()
                    if "=" in key_val:
