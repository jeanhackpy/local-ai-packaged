...
class Pipe:
    ...
    async def pipe(
        ...
        if selected_model.startswith("openrouter/"):
            ...
            # Note: We await the call_ollama because it's now an async method
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        # Fast-path: short queries are usually simple
        if len(query) < 15:
            return "Easy"
        ...
        try:
            payload = {
                ...
                "options": {
                    "temperature": 0,
                    "num_predict": 5  # Limit output to save time/resources
                }
            }
