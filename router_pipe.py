...
class Pipe:
    ...
    async def pipe(
        ...
        # Bolt Optimization: Fast-path for short queries (< 15 chars)
        elif len(user_query) < 15:
            selected_model = self.valves.easy_model
            route_reason = "Fast-path: Short query -> Local"
        else:
            # Evaluate difficulty
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        ...
        try:
            payload = {
                ...
                "options": {
                    "temperature": 0,
                    "num_predict": 5  # Bolt Optimization: Limit tokens for faster eval
                }
            }
            # Bolt Optimization: Offload blocking I/O to thread
