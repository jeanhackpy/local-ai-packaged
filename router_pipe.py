...
class Pipe:
    class Valves(BaseModel):
        ...
        fast_path_threshold: int = Field(
            default=15,
            description="Queries shorter than this bypass LLM evaluation and go to Easy model"
        )
    ...
    async def pipe(
        ...
        # Bolt Optimization: Use async calls to prevent event loop blocking
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        ...
        try:
            payload = {
                ...
                "options": {
                    "temperature": 0,
                    "num_predict": 5  # Bolt Optimization: Limit tokens for faster evaluation
                }
            }
            # Bolt Optimization: Offload blocking requests call to thread
