...
class Pipe:
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        ...
        try:
            ...
            response = await asyncio.to_thread(
                requests.post, f"{self.valves.ollama_url}/api/generate", json=payload, timeout=30
            )
    ...
    async def call_ollama(self, model: str, body: dict) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                requests.post, f"{self.valves.ollama_url}/api/chat", json=payload, timeout=30
            )
            if response.status_code == 200:
    ...
    async def call_openrouter(self, model: str, body: dict) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                requests.post, "https://openrouter.ai/api/v1/chat/completions", headers=headers, json=payload, timeout=30
            )
            if response.status_code == 200:
