...
class Pipe:
    ...
    async def pipe(
        ...
        if self.valves.privacy_mode:
            ...
            # Fast-path optimization: bypass LLM difficulty evaluation for short queries
            selected_model = self.valves.easy_model
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        ...
        try:
            payload = {
                ...
                "prompt": prompt,
                "stream": False,
                # Add num_predict to ensure early termination and reduce latency
                "options": {"temperature": 0, "num_predict": 5}
            }
            # Offload blocking request to a thread pool to avoid blocking the async event loop
    ...
    async def call_ollama(self, model: str, body: dict) -> str:
        ...
        try:
            # Offload blocking request to a thread pool
            response = await asyncio.to_thread(
                requests.post,
