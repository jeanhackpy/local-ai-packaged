...
class Pipe:
    ...
    async def evaluate_difficulty(self, query: str) -> str:
        # Fast-path heuristic: Short queries are usually easy and don't require LLM evaluation
        if len(query) < 15:
            return "Easy"
        ...
        try:
            payload = {
                ...
                "options": {
                    "temperature": 0,
                    "num_predict": 5  # Speed up inference by limiting tokens
                }
            }
    ...
    async def call_ollama(self, model: str, body: dict) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                ...
                json=payload,
                timeout=60  # Explicit timeout for model generation
            )
            if response.status_code == 200:
    ...
    async def call_openrouter(self, model: str, body: dict) -> str:
        ...
        try:
            response = await asyncio.to_thread(
                ...
                headers=headers,
                json=payload,
                timeout=60  # Explicit timeout for cloud models
            )
            if response.status_code == 200:
